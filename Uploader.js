...
const Uploader = () => {
  ...
  const Layout = ({
    ...
    return (
      <div>
        <div class="drag-and-drop-top">
          {previews.length > 0 ? <div>{previewDone}</div> : null}
        </div>
        <div {...dropzoneProps}>{files.length < maxFiles && input}</div>
        <div class="drag-and-drop-bottom">
          {previews.length > 0 ? <div>{previewNotDone}</div> : null}
        </div>
      </div>
    );
  };
      ...
      >
        {isRejected ? (
          ...
          <div className="container">
            ...
            <br />
            <label className="rounded" for="upload">
              Add Attachment
            </label>
          </div>
        )}
  ...
  const handleChangeStatus = (
    { meta, xhr, remove, cancel, restart, fileWithMeta },
    status,
    event
  ) => {
